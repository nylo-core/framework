...
Future<http.Response> metroApiRequest(NyApiRequest nyApiRequest) async {
  ...
  switch (nyApiRequest.method) {
    case "post": {
      response = await http.post(url, body: jsonEncode(nyApiRequest.data), headers: nyApiRequest.headerMap());
      break;
    }
    case "get": {
    ...
    case "put": {
      response = await http.put(url, headers: nyApiRequest.headerMap());
      break;
    }
    case "delete": {
